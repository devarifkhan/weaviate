...
func TestVectorDropIndexHelper_FindOrphanedVectorNames(t *testing.T) {
	...
	t.Run("no lsm directory - returns nil", func(t *testing.T) {
		indexPath, shardName := setup(t)

		orphaned, err := h.findOrphanedVectorNames(indexPath, shardName, nil)
		require.NoError(t, err)
		assert.Nil(t, orphaned)
	})

	t.Run("empty lsm directory - returns nil", func(t *testing.T) {
		indexPath, shardName := setup(t)
	...
	t.Run("all vectors configured - returns nil", func(t *testing.T) {
		indexPath, shardName := setup(t)
		...
		orphaned, err := h.findOrphanedVectorNames(indexPath, shardName, configured)
		require.NoError(t, err)
		assert.Nil(t, orphaned)
	})

	t.Run("some vectors orphaned", func(t *testing.T) {
		indexPath, shardName := setup(t)
		createLSMBucket(t, indexPath, shardName, "vectors_flat_bq")
		createLSMBucket(t, indexPath, shardName, "vectors_hnsw_rq8")
		createLSMBucket(t, indexPath, shardName, "vectors_compressed_flat_bq")
		// Also create non-vector entries that should be ignored
		createLSMBucket(t, indexPath, shardName, "objects")
		createLSMBucket(t, indexPath, shardName, "property_name")
		...
		orphaned, err := h.findOrphanedVectorNames(indexPath, shardName, configured)
		require.NoError(t, err)
		assert.Equal(t, []string{"hnsw_rq8"}, orphaned)
	})

	t.Run("all vectors orphaned", func(t *testing.T) {
		indexPath, shardName := setup(t)
		createLSMBucket(t, indexPath, shardName, "vectors_flat_bq")
		createLSMBucket(t, indexPath, shardName, "vectors_hnsw_rq8")

		orphaned, err := h.findOrphanedVectorNames(indexPath, shardName, map[string]struct{}{})
		require.NoError(t, err)
		assert.ElementsMatch(t, []string{"flat_bq", "hnsw_rq8"}, orphaned)
	})

	t.Run("skips compressed buckets", func(t *testing.T) {
		indexPath, shardName := setup(t)
		// Only compressed bucket, no vectors_ bucket
		createLSMBucket(t, indexPath, shardName, "vectors_compressed_flat_bq")
	...
	t.Run("discovers orphaned HNSW vectors from commit log dirs", func(t *testing.T) {
		indexPath, shardName := setup(t)
		// No LSM bucket, but HNSW commit log exists at shard level
		shardDir := filepath.Join(indexPath, shardName)
		require.NoError(t, os.MkdirAll(filepath.Join(shardDir, "vectors_hnsw_rq8.hnsw.commitlog.d"), 0o755))
		// Also create LSM dir so it exists
		require.NoError(t, os.MkdirAll(filepath.Join(shardDir, "lsm"), 0o755))
	...
	t.Run("deduplicates vectors found in both LSM and shard dirs", func(t *testing.T) {
		indexPath, shardName := setup(t)
		createLSMBucket(t, indexPath, shardName, "vectors_myvec")
		shardDir := filepath.Join(indexPath, shardName)
		require.NoError(t, os.MkdirAll(filepath.Join(shardDir, "vectors_myvec.hnsw.commitlog.d"), 0o755))

		orphaned, err := h.findOrphanedVectorNames(indexPath, shardName, map[string]struct{}{})
		require.NoError(t, err)
		assert.Equal(t, []string{"myvec"}, orphaned)
	})
}
...
func TestVectorDropIndexHelper_EnsureFilesAreRemovedForDroppedVectorIndexes(t *testing.T) {
	...
	t.Run("no vector config and no files - no error", func(t *testing.T) {
		indexPath, shardName := setup(t)
	...
	t.Run("all vectors still configured - files kept", func(t *testing.T) {
		indexPath, shardName := setup(t)
		createLSMBucket(t, indexPath, shardName, "vectors_flat_bq")
		createLSMBucket(t, indexPath, shardName, "vectors_compressed_flat_bq")
		createLSMBucket(t, indexPath, shardName, "vectors_hnsw_rq8")
	...
	t.Run("non-vector entries are not touched", func(t *testing.T) {
		indexPath, shardName := setup(t)
		...
		assert.True(t, pathExists(filepath.Join(indexPath, shardName, "lsm", "objects")))
		assert.True(t, pathExists(filepath.Join(indexPath, shardName, "lsm", "property_name")))
		assert.False(t, pathExists(filepath.Join(indexPath, shardName, "lsm", "vectors_dropped")))
	})
}
